...
class PPOAgent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.actor.load_weights("./saved_models/my_ppo_pendulum_actor.h5")
            self.critic.load_weights("./saved_models/my_ppo_pendulum_critic.h5")

    # approximate policy and value using Neural Network
    # actor: state is input and tanh activated action is output
    ...
    def train(self, state, action, reward, next_state, done, target, advantages):
        ...
        old_action = self.actor.predict(state, batch_size=1).flatten()
        old_aprob = old
        ...
        new_action = self.temp_actor.predict(state, batch_size=1).flatten()
        #new_aprob = new_policy[action]

        

        ratio = rescaled_new_action / rescaled_old_action
        # scale = min(ratio * advantages, K.clip(ratio, 1 - self.epsilon, 1 + self.epsilon) * advantages)
        no_clip = (ratio * old_action) * advantages
        clipped = (np.clip(ratio, 1 - self.epsilon, 1 + self.epsilon) * old_action) * advantages
    ...
    for episode in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                pylab.savefig("./save_graph/ppo_pendulum.png")
        ...
        # save model
        if episode % 50 == 0:
            agent.actor.save_weights("./saved_models/my_ppo_pendulum_actor.h5")
            agent.critic.save_weights("./saved_models/my_ppo_pendulum_critic.h5")
